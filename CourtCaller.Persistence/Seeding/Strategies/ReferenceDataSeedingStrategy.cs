...
namespace CourtCaller.Persistence.Seeding.Strategies
{
	...
	string rootPath)
		...
		public bool ShouldSeed(string environment)
		{
			...
			return settings.SeedReferenceData && !settings.ExcludedEntities.Contains("ReferenceData");
		}
		...
		public async Task<SeedingResult> SeedAsync(CancellationToken cancellationToken = default)
		{
			...
			try
			{
				...
				{
					("Branches", () => SeedEntityAsync<Branch>(AppCts.SeederRelativePath.BranchPath, result, cancellationToken)),
					("Courts", () => SeedEntityAsync<Court>(AppCts.SeederRelativePath.CourtPath, result, cancellationToken)),
					("Prices", () => SeedEntityAsync<Price>(AppCts.SeederRelativePath.PricePath, result, cancellationToken)),
					("News", () => SeedEntityAsync<News>(AppCts.SeederRelativePath.NewsPath, result, cancellationToken))
				};
