	...
	public class CoreDataSeedingStrategy : ISeedingStrategy
	{
		...
		public async Task<SeedingResult> SeedAsync(CancellationToken cancellationToken = default)
		{
			...
			try
			{
				...
				_logger.LogInformation("Core data seeding completed in {Duration}ms", stopwatch.ElapsedMilliseconds);
			}
			catch (Exception ex)
			{
		...
		private async Task SeedIdentityRolesAsync(SeedingResult result, CancellationToken cancellationToken)
		{
			var existingRolesCount = await _context.Roles.CountAsync(cancellationToken);

			if (existingRolesCount > 0 && !_config.ForceReseed)
			{
				_logger.LogInformation("Skipping role seeding: {Count} roles already exist", existingRolesCount);
				return;
			}
		...
		private async Task SeedDefaultAdminUserAsync(SeedingResult result, CancellationToken cancellationToken)
		{
			// Check if any admin users exist
			var adminRole = await _context.Roles.FirstOrDefaultAsync(r => r.NormalizedName == "ADMIN", cancellationToken);
			if (adminRole == null)
			{
			...
			_logger.LogInformation("No admin users found. In production, consider creating a default admin user through a separate secure process");
