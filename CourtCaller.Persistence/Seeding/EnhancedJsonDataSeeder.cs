...
{
    public class EnhancedJsonDataSeeder<T>
        where T : class
    {
        ...
        public EnhancedJsonDataSeeder<T> AddRelativeFilePath(
            string basePath,
        ...
        )
        {
            ...
            try
            {
                ...
                if (skipIfExists && existingCount > 0 && !_config.ForceReseed)
                {
                    ...
                    result.Message =
                        $"Skipping {entityName}: {existingCount} records already exist";
                    _logger.LogInformation(
                        "Skipping {EntityName}: {Count} records already exist",
                        entityName,
                        existingCount
                    );
                    return result;
                ...
                {
                    _logger.LogWarning(
                        "Force reseed enabled. Removing {Count} existing {EntityName} records",
                        existingCount,
                        entityName
                    );
                ...
                _logger.LogInformation(
                    "Seeding {TotalRecords} {EntityName} records in {BatchCount} batches of {BatchSize}",
                    totalRecords,
                    entityName,
                    batches.Count,
                    batchSize
                ...
                try
                {
                    ...
                    {
                        _logger.LogDebug(
                            "Processing batch {BatchIndex}/{TotalBatches} for {EntityName}",
                            index + 1,
                            batches.Count,
                            entityName
                        );
                    ...
                    result.Message =
                        $"Successfully seeded {result.RecordsSeeded} {entityName} records";

                    _logger.LogInformation(
                        "Successfully seeded {RecordsSeeded} {EntityName} records",
        ...
        private async Task<IEnumerable<T>> ParseAndValidateJsonAsync(
        ...
        )
        {
            try
            {
                ...
                _logger.LogInformation(
                    "Parsed {ValidCount}/{TotalCount} valid {EntityName} records",
                    validatedData.Count,
                    data.Count(),
                    typeof(T).Name
                );
            ...
            catch (JsonException ex)
            {
                throw new InvalidOperationException(
                    $"Invalid JSON format in {_absoluteFilePathJson}: {ex.Message}",
                    ex
                );
            }
            catch (Exception ex)
            {
                throw new InvalidOperationException(
                    $"Failed to parse JSON from {_absoluteFilePathJson}: {ex.Message}",
                    ex
                );
            }
        }
