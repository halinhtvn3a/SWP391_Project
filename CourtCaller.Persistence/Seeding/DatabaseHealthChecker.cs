...
namespace CourtCaller.Persistence.Seeding
{
	...
	public class DatabaseHealthChecker : IDatabaseHealthChecker
	{
		...
		public DatabaseHealthChecker(CourtCallerDbContext context, ILogger<DatabaseHealthChecker> logger)
		{
			_context = context;
			_logger = logger;
		}
		...
		public async Task<DatabaseHealthResult> CheckHealthAsync(CancellationToken cancellationToken = default)
		{
			...
			try
			{
				...
				{
					result.Issues.Add($"Found {result.PendingMigrations.Count} pending migrations: {string.Join(", ", result.PendingMigrations)}");
				}

				// Check 3: Database exists and tables are created
				result.DatabaseExists = await _context.Database.EnsureCreatedAsync(cancellationToken) == false; // false means it already existed
				...
				foreach (var (tableName, checkFunc) in tableChecks)
				{
					...
					{
						result.Issues.Add($"Table {tableName} does not exist or is not accessible: {ex.Message}");
				...
				_logger.LogInformation("Database health check completed in {Duration}ms. Status: {Status}",
					result.CheckDuration.TotalMilliseconds,
					result.IsHealthy ? "Healthy" : "Unhealthy");

				if (result.Issues.Any())
				{
					_logger.LogWarning("Database health issues found: {Issues}", string.Join("; ", result.Issues));
				}
