...
namespace CourtCaller.Persistence.Seeding
{
	...
	public class EnterpriseSeedingManager : IEnterpriseSeedingManager
	{
		...
		public async Task<SeedingResult> ExecuteSeedingAsync(CancellationToken cancellationToken = default)
		{
			...
			try
			{
				...
				_logger.LogInformation("Seeding Configuration: Enabled={Enabled}, ForceReseed={ForceReseed}, ValidateAfterSeed={ValidateAfterSeed}",
					_config.Enabled, _config.ForceReseed, _config.ValidateAfterSeed);
				...
				{
					overallResult.Errors.Add($"Database health check failed: {string.Join(", ", healthResult.Issues)}");

					// If we have pending migrations, try to apply them
					if (healthResult.HasPendingMigrations)
					{
						_logger.LogWarning("Found {Count} pending migrations. Attempting to apply...", healthResult.PendingMigrations.Count);
						try
						{
				...
				if (_config.EnableBackup && _environment.Equals("Production", StringComparison.OrdinalIgnoreCase))
				{
				...
				_logger.LogInformation("Found {Count} applicable seeding strategies: {Strategies}",
				...
				{
					_logger.LogInformation("Executing strategy: {StrategyName} (Priority: {Priority})",
					...
					while (retryCount <= _config.MaxRetryAttempts)
					{
						try
						{
							strategyResult = await strategy.SeedAsync(cancellationTokenSource.Token);
							break;
						}
						catch (Exception ex) when (retryCount < _config.MaxRetryAttempts)
						{
							retryCount++;
							_logger.LogWarning(ex, "Strategy {StrategyName} failed on attempt {Attempt}/{MaxAttempts}. Retrying...",
								strategy.Name, retryCount, _config.MaxRetryAttempts + 1);
						...
						{
							_logger.LogError(ex, "Strategy {StrategyName} failed after {MaxAttempts} attempts",
								strategy.Name, _config.MaxRetryAttempts + 1);
					...
					{
						_logger.LogError("Strategy {StrategyName} failed: {Errors}",
							strategy.Name, string.Join(", ", strategyResult.Errors));
					...
					{
						_logger.LogInformation("Strategy {StrategyName} completed successfully. Records seeded: {RecordsSeeded}",
							strategy.Name, strategyResult.RecordsSeeded);
					}
				}
				...
				_logger.LogInformation("Success: {Success}, Total Records: {RecordsSeeded}, Duration: {Duration}ms",
					overallResult.IsSuccess, overallResult.RecordsSeeded, stopwatch.ElapsedMilliseconds);

				if (overallResult.Errors.Any())
				{
					_logger.LogWarning("Seeding completed with errors: {Errors}", string.Join("; ", overallResult.Errors));
				}
			}
			catch (Exception ex)
			{
		...
		private async Task<List<string>> ValidateSeededDataAsync(CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				var orphanedCourts = await _context.Courts
					.Where(c => !_context.Branches.Any(b => b.BranchId == c.BranchId))
				...
				var orphanedTimeSlots = await _context.TimeSlots
					.Where(ts => !_context.Courts.Any(c => c.CourtId == ts.CourtId))
					.CountAsync(cancellationToken);

				if (orphanedTimeSlots > 0)
				{
					errors.Add($"Found {orphanedTimeSlots} time slots without valid court references");
				}

				// Check for required roles
				var adminRoleExists = await _context.Roles.AnyAsync(r => r.NormalizedName == "ADMIN", cancellationToken);
				if (!adminRoleExists)
				{
