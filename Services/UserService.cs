...
namespace Services
{
    public class UserService
    {
        ...
        public void UpdateIdentityUser(IdentityUser IdentityUser) =>
            _userRepository.UpdateIdentityUser(IdentityUser);
        ...
        )
        {
            ...
            string key =
                $"all_users_pageNum_{page.PageNumber}_pageSize_{page.PageSize}_Search_{searchQuery}";
            ...
            {
                var cachedData = JsonConvert.DeserializeObject<(List<IdentityUser>, int)>(
                    cachedUsers
                );
        ...
        public List<IdentityUser> SearchUserByEmail(string searchValue) =>
            _userRepository.SearchUserByEmail(searchValue);
        ...
        {
            bool isBlackListed =
                _db.StringGet(token) == "BlackList" || _db.StringGet(refreshToken) == "BlackList";
