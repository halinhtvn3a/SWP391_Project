...
namespace Services
{
    public class TrainingService
    {
        ...
        public async Task<(
            ...
            float growthRate
        )> PredictWeeklyBookingGrowthAsync()
        {
            try
            {
                ...
                for (int i = 0; i < 7; i++)
                {
                    ...
                    var numberOfSlots = await _timeSlotRepository.GetNumberOfSlotsForDateAsync(
                        nextWeekDate
                    );
                    ...
                    Console.WriteLine(
                        $"Predicting for: DayOfWeek={bookingData.DayOfWeek}, TimeOfDay={bookingData.TimeOfDay}, Court={bookingData.Court}, NumberOfSlot={bookingData.NumberOfSlot}"
                    );
                ...
                Console.WriteLine(
                    $"Next week predicted count: {nextWeekPredictedCount}, Growth rate: {growthRate}"
                );
