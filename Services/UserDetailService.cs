...
namespace Services
{
    public class UserDetailService
    {
        ...
        public UserDetail UpdateUserDetail(string id, UserDetailsModel userDetailsModel) =>
            _userDetailRepository.UpdateUserDetail(id, userDetailsModel);

        public async Task<UserDetail> UpdateUserDetailAsync(string userId) =>
            await _userDetailRepository.UpdateUserDetailAsync(userId);

        public UserDetail UpdateUser(string id, PutUserDetail putUserDetail) =>
            _userDetailRepository.UpdateUser(id, putUserDetail);

        public List<UserDetail> SearchUserByEmail(string searchValue) =>
            _userDetailRepository.SearchUserByEmail(searchValue);

        public async Task<List<UserDetail>> SortUserDetail(
            string? sortBy,
            bool isAsc,
            PageResult pageResult
        ) => await _userDetailRepository.SortUserDetail(sortBy, isAsc, pageResult);
