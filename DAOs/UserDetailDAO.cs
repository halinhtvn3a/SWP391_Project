    ...
    public class UserDetailDAO
    {
        ...
        public List<UserDetail> SearchUserByEmail(string searchValue)
        {
            try
            {
                ...
                var userDetail = _dbContext
                    .UserDetails.Where(u => u.UserId.Equals(userId))
                    .ToList();
                return userDetail;
            }
            catch (Exception ex)
        ...
        )
        {
            ...
            switch (sortBy?.ToLower())
            {
                ...
                case "address":
                    query = isAsc
                        ? query.OrderBy(b => b.Address)
                        : query.OrderByDescending(b => b.Address);
                    break;
                case "Point":
                    query = isAsc
                        ? query.OrderBy(b => b.Point)
                        : query.OrderByDescending(b => b.Point);
                    break;
                case "fullname":
                    query = isAsc
                ...
                case "yearofbirth":
                    query = isAsc
                        ? query.OrderBy(b => b.YearOfBirth)
                        : query.OrderByDescending(b => b.YearOfBirth);
                    break;
                default:
                    break;
            }
            Pagination pagination = new Pagination(_dbContext);
            List<UserDetail> userDetails = await pagination.GetListAsync<UserDetail>(
                query,
                pageResult
            );
            return userDetails;
