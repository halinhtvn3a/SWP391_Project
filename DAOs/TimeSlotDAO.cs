...
namespace DAOs
{
    public class TimeSlotDAO
    {
        ...
        )
        {
            ...
            {
                query = query.Where(timeslot =>
                    timeslot.SlotId.Contains(searchQuery)
        ...
        public async Task<List<TimeSlot>> GetTimeSlotsByCourtId(
            string courtId,
            PageResult pageResult,
            string searchQuery = null
        )
        ...
        {
            TimeSlot timeslots = _dbContext
                .TimeSlots.Where(t => t.SlotId.Equals(id))
                .FirstOrDefault();
            return timeslots;
        }
        ...
        public bool IsSlotAvailable(SlotModel slotModel)
        {
            ...
            {
                if (
                    slot.SlotStartTime.Equals(slotModel.TimeSlot.SlotStartTime)
                    && slot.SlotEndTime.Equals(slotModel.TimeSlot.SlotEndTime)
                )
                {
                    return false;
                }
        ...
        public TimeSlot AddSlotToBooking(SlotModel slotModel, string bookingId)
        {
            ...
            TimeSlot timeSlot = new TimeSlot
            {
                ...
                CourtId =
                    ...
                    ?? _dbContext
                        .Courts.Where(c =>
                            c.BranchId == slotModel.BranchId
                            && !_dbContext.TimeSlots.Any(ts =>
                                ts.CourtId == c.CourtId
                                && ts.SlotDate == slotModel.SlotDate
                                && (
                                    (
                                        ts.SlotStartTime <= slotModel.TimeSlot.SlotStartTime
                                        && ts.SlotEndTime > slotModel.TimeSlot.SlotStartTime
                                    )
                                    || (
                                        ts.SlotStartTime < slotModel.TimeSlot.SlotEndTime
        ...
        public async Task DeleteBookingAndSetTimeSlotAsync(string bookingId)
        {
            using (var transaction = await _dbContext.Database.BeginTransactionAsync())
            {
                try
                {
                    var timeSlots = await _dbContext
                        .TimeSlots.Where(ts => ts.BookingId == bookingId && ts.SlotDate != null) // Kiểm tra trường DateTimeColumn không null
                        .ToListAsync();
        ...
        {
            bool isWeekend =
                slotModel.SlotDate.DayOfWeek == DayOfWeek.Saturday
                || slotModel.SlotDate.DayOfWeek == DayOfWeek.Sunday;

            Price pricing = null;
            if (slotModel.BranchId != null)
            {
                pricing = _dbContext.Prices.FirstOrDefault(p =>
                    p.BranchId == slotModel.BranchId && p.IsWeekend == isWeekend
                );
            }
            else
            {
                ...
                if (court != null)
                {
                    var branch = _dbContext.Branches.FirstOrDefault(b =>
                        b.BranchId == court.BranchId
                    );
                    if (branch != null)
                    {
                        pricing = _dbContext.Prices.FirstOrDefault(p =>
                            p.BranchId == branch.BranchId && p.IsWeekend == isWeekend
                        );
                    }
                }
            }
        ...
        public async Task<List<TimeSlot>> GetTimeSlotsByUserId(string userId, PageResult pageResult)
        {
            var query = _dbContext
                .TimeSlots.Where(ts =>
                    _dbContext.Bookings.Any(b => b.BookingId == ts.BookingId && b.Id == userId)
                )
        ...
        public async Task<List<TimeSlot>> SortTimeSlot(
            string? sortBy,
            bool isAsc,
            PageResult pageResult
        )
        {
            ...
            switch (sortBy?.ToLower())
            {
                case "slotid":
                    query = isAsc
                        ? query.OrderBy(b => b.SlotId)
                        : query.OrderByDescending(b => b.SlotId);
                    break;
                case "bookingid":
                    query = isAsc
                ...
                case "courtid":
                    query = isAsc
                        ? query.OrderBy(b => b.CourtId)
                        : query.OrderByDescending(b => b.CourtId);
                    break;
                case "slotdate":
                    query = isAsc
                        ? query.OrderBy(b => b.SlotDate)
                        : query.OrderByDescending(b => b.SlotDate);
                    break;
                case "slotstarttime":
                    query = isAsc
                        ? query.OrderBy(b => b.SlotStartTime)
                        : query.OrderByDescending(b => b.SlotStartTime);
                    break;
                case "slotendtime":
                    query = isAsc
                        ? query.OrderBy(b => b.SlotEndTime)
                        : query.OrderByDescending(b => b.SlotEndTime);
                    break;
                case "price":
                    query = isAsc
        ...
        {
            var count =
                _dbContext.Courts.Count(c => c.BranchId == slotCheckModel.BranchId)
                - _dbContext.TimeSlots.Count(ts =>
                    ts.Court.BranchId == slotCheckModel.BranchId
                    && ts.SlotDate == slotCheckModel.SlotDate
                    && ts.SlotStartTime == slotCheckModel.TimeSlot.SlotStartTime
                    && ts.SlotEndTime == slotCheckModel.TimeSlot.SlotEndTime
                );
            return count;
        }
        ...
        {
            var courtIds = _dbContext
                .Courts.Where(c => c.BranchId == branchId)
                .Select(c => c.CourtId)
                .ToList();
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                var allTimeSlots = _dbContext
                    .TimeSlots.Where(ts => ts.SlotDate == currentday)
                    .AsEnumerable()
                ...
                foreach (var timeslot in allTimeSlots)
                {
                    ...
                    {
                        if (
                            !unavailableSlots.Any(us =>
                                us.SlotStartTime == timeslot.SlotStartTime
                                && us.SlotEndTime == timeslot.SlotEndTime
                                && us.SlotDate == timeslot.SlotDate
                            )
                        )
                        {
        ...
        public async Task<int> GetNumberOfSlotsForDateAsync(DateTime date)
        {
            ...
            var slotCount = await _dbContext
                .TimeSlots.Where(ts => ts.SlotDate == dateOnly)
                .CountAsync();
