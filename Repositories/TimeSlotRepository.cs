...
namespace Repositories
{
    public class TimeSlotRepository
    {
        ...
        public async Task<List<TimeSlot>> GetTimeSlots(
            PageResult pageResult,
            string searchQuery = null
        ) => await _timeSlotDao.GetTimeSlots(pageResult, searchQuery);

        public async Task<List<TimeSlot>> GetTimeSlotsByUserId(
            string userId,
            PageResult pageResult
        ) => await _timeSlotDao.GetTimeSlotsByUserId(userId, pageResult);

        public List<TimeSlot> GetTimeSlotsByDate(DateOnly dateOnly) =>
            _timeSlotDao.GetTimeSlotsByDate(dateOnly);
        ...
        public async Task UpdateTimeSlotWithObject(TimeSlot timeSlot) =>
            await _timeSlotDao.UpdateTimeSlotWithObject(timeSlot);

        public async Task<TimeSlot> UpdateTimeSlot(string id, SlotModel slotModel) =>
            await _timeSlotDao.UpdateTimeSlot(id, slotModel);
        ...
        public bool IsSlotBookedInBranch(SlotModel slotModel)
        {
            ...
            {
                List<Court> courtsInBranch = _courtDao
                    .GetCourts()
                    .Where(c => c.BranchId == slotModel.BranchId && c.Status == "Active")
                    .ToList();
                foreach (var court in courtsInBranch)
                {
                    isBooked = false;
                    List<TimeSlot> timeSlots = _timeSlotDao
                        .GetTimeSlots()
                    ...
                    {
                        if (
                            slotModel.SlotDate == timeSlot.SlotDate
        ...
        public List<TimeSlot> GetTimeSlotsByBookingId(string bookingId) =>
            _timeSlotDao.GetTimeSlotsByBookingId(bookingId);
        ...
        public TimeSlot ChangeSlot(SlotModel slotModel, string slotId)
        {
            ...
            var timeSlot = _timeSlotDao.GetTimeSlots().FirstOrDefault(t => t.SlotId == slotId);
        ...
        public async Task<List<TimeSlot>> SortTimeSlot(
            ...
            bool isAsc,
            PageResult pageResult
        ) => await _timeSlotDao.SortTimeSlot(sortBy, isAsc, pageResult);

        public int CountTimeSlot(SlotCheckModel slotCheckModel) =>
            _timeSlotDao.CountTimeSlot(slotCheckModel);

        public List<TimeSlotModel> UnavailableSlot(DateOnly date, string branchId) =>
            _timeSlotDao.UnavailableSlot(date, branchId);
        ...
        public async Task<int> GetNumberOfSlotsForDateAsync(DateTime date) =>
            await _timeSlotDao.GetNumberOfSlotsForDateAsync(date);
    }
}
