...
namespace Repositories
{
    public class ReviewRepository
    {
        ...
        public Review UpdateReview(string id, ReviewModel reviewModel) =>
            _reviewDao.UpdateReview(id, reviewModel);
        ...
        public List<Review> SearchByDate(DateTime start, DateTime end) =>
            _reviewDao.SearchByDate(start, end);
        ...
        public async Task<List<Review>> SortReview(
            ...
            bool isAsc,
            PageResult pageResult
        ) => await _reviewDao.SortReview(sortBy, isAsc, pageResult);
        ...
        public List<decimal> GetRatingPercentageOfABranch(string branchId)
        {
            ...
            for (int i = 1; i < 6; i++)
            {
                ...
                {
                    var listOfRatingRate =
                        (totalReviews / _reviewDao.GetReviewsByBranch(branchId).Count()) * 100;
